...
const tabs = ["Latest News", "School Calendar", "Downloads"];
const tabMap: Record<string, number> = { latest: 0, calendar: 1, holidays: 1, downloads: 2 };
...
export function ResourcesClient({ news, documents, calendar, calendarSource }: {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setActive(selected ? (tabMap[selected] ?? 0) : 0), 0);
  ...
  return <>
    ...
    <section className="bg-white py-16 md:py-20">
      <div className="mx-auto max-w-7xl px-6">
        ...
        {active === 1 && <SchoolCalendarTab data={calendar} source={calendarSource} />}
        {active === 2 && <DownloadList items={documents} />}
      </div>
    </section>
  </>;
}
